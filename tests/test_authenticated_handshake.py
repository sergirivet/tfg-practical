...
from primitives.kem.classical import dh_keygen, dh_shared_secret, kem_keygen, kem_encapsulate, kem_decapsulate
from primitives.kem.quantum import kyber_keygen, kyber_encapsulate, kyber_decapsulate
from protocol.hybrid_handshake import (
    hybrid_session_key,
...
def test_authenticated_hybrid_handshake():
    ...
    # =========================================================================
    print("\n--- Phase 0: Server Long-Term Key Generation (DUAL KEYS) ---")
    print("(This happens once during server setup, not per-session)")
    
    # Server generates BOTH classical and post-quantum signing key pairs
    # The public keys would be distributed via certificates in a real system
    ...
    print(f"Server Dilithium signing public key: {server_signing_public_dilithium[:32].hex()}... (truncated)")
    print(f"Server ECDSA signing public key: {server_signing_public_ecdsa[:50].hex()}... (truncated)")
    print(f"Server signing private keys: [KEPT SECRET]")
    ...
    # Server signs the handshake transcript with BOTH schemes
    # Signature proves server's identity and binds all four ephemeral public keys
    signature_dilithium, signature_ecdsa = server_sign_handshake(
    ...
    print(f"Server Dilithium signature: {signature_dilithium[:32].hex()}... (truncated)")
    print(f"Server ECDSA signature: {signature_ecdsa[:32].hex()}... (truncated)")
    
    print("Server sends to Client: (pk_dh_s, pk_kyber_s, sig_dilithium, sig_ecdsa)")
    
    # =========================================================================
    # PHASE 3: Client verifies DUAL signatures before deriving secrets
    # =========================================================================
    ...
    try:
        ...
        print("✓ Dilithium (post-quantum) signature verification PASSED")
        print("✓ ECDSA (classical) signature verification PASSED")
...
def test_authentication_failure():
    ...
    # Server's legitimate long-term keys (DUAL KEYS for defense-in-depth)
    server_signing_public_dil, server_signing_private_dil = generate_keypair_dilithium()
    server_signing_public_ecdsa, server_signing_private_ecdsa = generate_keypair_ecdsa()
    
    # Attacker's keys (trying to impersonate server with DUAL KEYS)
    attacker_signing_public_dil, attacker_signing_private_dil = generate_keypair_dilithium()
    ...
    # Sign with BOTH schemes as if they were the legitimate server
    attacker_signature_dil, attacker_signature_ecdsa = server_sign_handshake(
        attacker_signing_private_dil,   # Attacker's Dilithium key
        attacker_signing_private_ecdsa,  # Attacker's ECDSA key
        client_dh_public, client_kyber_public,
    ...
    try:
        client_verify_handshake(
            server_signing_public_dil,      # Client uses legitimate server's Dilithium public key
            server_signing_public_ecdsa,    # Client uses legitimate server's ECDSA public key
            client_dh_public, client_kyber_public,
            attacker_dh_public, attacker_kyber_public,  # But keys are from attacker
            attacker_signature_dil, attacker_signature_ecdsa  # Signatures from attacker
        )
...
def test_authenticated_hybrid_session_key_function():
    ...
    # Server signs with BOTH schemes
    signature_dil, signature_ecdsa = server_sign_handshake(
        server_signing_private_dil,
        server_signing_private_ecdsa,
        client_dh_public, client_kyber_public,
        server_dh_public, server_kyber_public
    )
    ...
    try:
        session_key = authenticated_hybrid_session_key(
            ...
            server_signing_public_dil,
            server_signing_public_ecdsa,
            client_dh_public, client_kyber_public,
            server_dh_public, server_kyber_public,
            signature_dil, signature_ecdsa
        )
